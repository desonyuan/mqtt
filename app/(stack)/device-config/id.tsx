...
const DeviceConfigPage = () => {
  ...
  const fetchDeviceConfig = useCallback(async () => {
    ...
    try {
      ...
      // 订阅配置响应
      mqttService.subscribeDeviceConfig(master_device_uuid, handleConfigResult);
      
      // 请求设备配置
      mqttService.requestDeviceConfig(master_device_uuid, device_uuid);
      
      // 设置超时重试 - 使用 refs 访问最新状态
      timeoutRef.current = setTimeout(() => {
        // 使用 refs 获取最新值
        if (!configRef.current && retryCountRef.current < 3) {
          // 仅更新重试计数，不递归调用
  ...
  }, [master_device_uuid, device_uuid]);
  ...
  const processRemoveSlave = async (deviceUuid: string): Promise<boolean> => {
    ...
    try {
      ...
      console.log('移除从设备订阅响应成功');
      await new Promise(resolve => setTimeout(resolve, 3000));
      // 请求设备配置
      mqttService.requestDeviceConfig(master_device_uuid!, deviceUuid);
      console.log('移除从设备请求配置成功');
      // 等待配置响应，最多10秒
      while (tempConfig === undefined && retryCount < 3) {
        await new Promise(resolve => setTimeout(resolve, 4000));
        retryCount++;
        mqttService.requestDeviceConfig(master_device_uuid!, deviceUuid);
        console.log('移除从设备请求配置第', retryCount, '次');
        if (retryCount === 3 && tempConfig === undefined) {
          throw new Error(`获取设备${deviceUuid}配置失败`);
        }
      }
      console.log('移除从设备等待配置响应完成');
      // 如果是false，表示失败
      if (tempConfig === false) {
        throw new Error(`设备${deviceUuid}配置获取失败`);
      }
      console.log('移除从设备修改为主设备配置');
      // 修改为主设备配置
      tempConfig.masterUuid = null;
      tempConfig.deviceMode = DeviceMode.MASTER;
      console.log('移除从设备修改为主设备配置完成');
      // 发送配置
      const configPayload = tempConfig;
      mqttService.sendDeviceConfig(master_device_uuid!, configPayload);
      console.log('移除从设备发送配置成功');
      // 等待响应，最多10秒
      retryCount = 0;
      while (tempResponse === undefined && retryCount < 20) {
        await new Promise(resolve => setTimeout(resolve, 1000));
        retryCount++;
        console.log('移除从设备发送配置第', retryCount, '次');
        if (retryCount === 20 && tempResponse === undefined) {
          throw new Error(`设备${deviceUuid}配置发送超时`);
        }
      }
      console.log('移除从设备等待响应完成');
      // 如果是false，表示失败
      if (tempResponse === false) {
        throw new Error(`设备${deviceUuid}配置发送失败`);
      }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      console.log('successfullyRemoved', successfullyRemoved);
      // 合并从设备列表
      const newSlaves = [
        ...successfullyAdded,
        ...currentSlaves.filter(slave => !successfullyRemoved.includes(slave))
      ];
      console.log('currentSlaves', currentSlaves);
      console.log('合并从设备列表完成');
      console.log('newSlaves', newSlaves);
      // 更新主设备配置
      const finalConfig = {
        ...config,
        slave_uuids: newSlaves
      };
      console.log('更新主设备配置完成');
      // 重新连接MQTT
      await reconnectMqtt();
      console.log('重新连接MQTT成功');
